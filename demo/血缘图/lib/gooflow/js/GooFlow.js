...
//初始化区域图的对象
	...
	if(this.$editable){
	  ...
	  this.$textArea=$("<textarea id='testtttt'></textarea>");
...
GooFlow.prototype={
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf("round")<0){
			...
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+" item-"+nodeType+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'>"+
				"<table nodeType='"+nodeType+"' class='table-"+nodeType+"' cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'>"+
				"<tr><td class='ico'><b class='ico_"+json.type+"'></b></td><td>"+json.name+"</td></tr></table>"+
				"<div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","click",{inthis:this},function(e){
			let nType = $("#"+this.id).find('table').attr('nodetype');
			let nName = $("#"+this.id).find('.ico + td').text();
			console.log("当前点击节点的类型:"+nType);
			console.log("当前点击节点的名称:"+nName);
			// 改变vue中定义的nType值,可以在vue中通过监听nType的值动态切换相应表单
			vm.nType = nType;
			vm.nName = nName;
			if (nName == '开始' || nName == '结束') {
				return;
			}
			e.data.inthis.focusItem(this.id,true);
		...
		//绑定结点的右击弹出事件
		this.$workArea.delegate(".ico + td","contextmenu",{inthis:this},function(e){		
			var id = $(this).parents(".GooFlow_item").attr("id");
			var type = $("#"+id+" > table").attr("nodetype");
			e.preventDefault();
			showContextMenu(id,type,e);
		});
		//绑定结点的删除功能
		...
		//绑定自定义右击删除功能
		this.$workArea.delegate(".del-btn","click",{inthis:this},function(e){
			if(!e)e=window.event;
			let id = $(this).attr('pid');
			console.log("删除了id为:"+id+"的节点");
			$('.context-menu').remove();
			e.data.inthis.delNode(id);
			return false;
		});
		// 绑定自定义右击复制功能
		this.$workArea.delegate(".copy-btn","click",{inthis:this},function(e){
			if(!e)e=window.event;
			let id = $(this).attr('pid');
			let type = $(this).attr('nType');
			console.log("复制的节点id为:"+id);
			console.log("复制的节点类型为:"+type);
			// 复制节点后重新定义左侧按钮栏的选择节点类型(下次点击自动生成复制的节点类型)
			var This=e.data.inthis;
			This.switchToolBtn(type);
			// 自行添加复制业务
			vm.Message('复制成功,单击任意区域即可粘贴',true);
			$('.context-menu').remove();
			return false;
		});
		...
		/**
		 * --by LJX
		 * 绑定各监控组件节点的复选框Change事件，控制节点的可监控状态
		 */
		this.$workArea.delegate(".enableState","change",{inthis:this},function(e){
			e.stopPropagation();

			if(!e)e=window.event;
			var _self=$(this);
			_self.parent().toggleClass('active');
			// var id = _self.parents(".GooFlow_item").attr("id");
			
			// var enablestate=_self.is(":checked")?1:0;
			
			// e.data.inthis.$nodeData[id]["enableState"]=enablestate;
			// var obj=findObj(id);
			// if(obj){
			// 	obj["enablestate"]=enablestate;
			// }
			return false;
		});
	},
			...
			if(jq.prop("tagName")=="DIV"){
				...
				if(GooFlow.prototype.useSVG!=""){
					if(!this.$lineData[this.$focus].marked){
						jq[0].childNodes[1].setAttribute("stroke","#228adc");
						jq[0].childNodes[1].setAttribute("marker-end","url(#arrow1)");
					}
				}
				else{
					if(!this.$lineData[this.$focus].marked)	jq[0].strokeColor="#228adc";
				}
